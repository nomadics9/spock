...
{#if $currentUser && !loading}
	...
	/>
	<button on:click={signOut} class="btn btn-sm variant-filled-warning">Log out</button>
{/if}
{#if !$currentUser}
	<h1>Please Login</h1>
	<button
		class="btn size-full variant-soft-success rounded-xl"
		on:click={() => {
			login('google');
		}}
		><svg
